import { useEffect, useState, useRef, useMemo, MouseEvent as ReactMouseEvent, TouchEvent as ReactTouchEvent, ReactNode, createContext, useContext } from 'react';
...
export const translations = {
  pl: {
    ...
    food: "POTRAWY",
    cocktails: "KOKTAJLE",
    gallery_dishes: "POTRAWY",
    gallery_cocktails: "KOKTAJLE",
    gallery_interiors: "WNĘTRZE",
    view_menu: "ZOBACZ MENU",
    see_menu_card: "KARTA MENU",
    book_table: "Zarezerwuj Miejsce",
    book_table_upper: "ZAREZERWUJ MIEJSCE",
    events_btn: "Eventy",
    events_modal_title: "Organizacja eventów",
    events_modal_desc_1: "W tym miejscu może pojawić się formularz zgłoszeniowy do organizacji dużych eventów lub zaproponowana przez Pawła oferta PDF, spójna wizualnie ze stroną restauracji Nola.",
    ...
    people_few: "Gości",
    people_many: "Gości",
    more_people_contact: "w przypadku większej liczby Gości prosimy o kontakt telefoniczny.",
    ...
    recipient_locked_info: "Przypisany na stałe",
    sender_email: "Twój adres e-mail (do odpowiedzi)",
    email_subject: "Temat wiadomości",
    email_subject_placeholder: "Wpisz temat...",
    email_content: "Treść wiadomości",
    email_content_placeholder: "Wpisz treść wiadomości...",
    send_upper: "WYŚLIJ",
    message_sent_title: "Wiadomość wysłana!",
    message_sent_desc: "Dziękujemy za kontakt. Twoja wiadomość została przekazana, odpowiemy najszybciej jak to możliwe.",
  ...
  en: {
    ...
    cocktails: "COCKTAILS",
    gallery_dishes: "FOOD",
    gallery_cocktails: "COCKTAILS",
    gallery_interiors: "INTERIOR",
    view_menu: "Menu",
    see_menu_card: "MENU",
    book_table: "Book a Table",
    book_table_upper: "BOOK A TABLE",
    events_btn: "Events",
    events_modal_title: "Events Organization",
    events_modal_desc_1: "An inquiry form for organizing large events or a PDF offer proposed by Paweł, visually consistent with the Nola Restaurant website, may appear here.",
    ...
    people_few: "Guests",
    people_many: "Guests",
    more_people_contact: "in case of a larger number of Guests, please contact us by phone.",
    ...
    recipient_locked_info: "Permanently assigned",
    sender_email: "Your email address (for reply)",
    email_subject: "Subject",
    email_subject_placeholder: "Enter subject...",
    email_content: "Message",
    email_content_placeholder: "Enter message...",
    send_upper: "SEND",
    message_sent_title: "Message Sent!",
    message_sent_desc: "Thank you for contacting us. Your message has been sent, we will reply as soon as possible.",
...
const InteractiveMenuPlate = ({
  ...
  const handleMouseMove = (e: ReactMouseEvent<HTMLDivElement>) => {
    if (!containerRef.current || !movingLayerRef.current) return;
    const rect = containerRef.current.getBoundingClientRect();
    const x = e.clientX - rect.left - rect.width / 2;
    const y = e.clientY - rect.top - rect.height / 2;
    // Calculate an expressive, lively pull with smooth subpixel precision
    const pullX = Number((x * 0.075).toFixed(2));
    const pullY = Number((y * 0.075).toFixed(2));
    
    if (rafRef.current) cancelAnimationFrame(rafRef.current);
    rafRef.current = requestAnimationFrame(() => {
      if (movingLayerRef.current) {
        movingLayerRef.current.style.transform = `translate3d(${pullX}px, ${pullY}px, 0)`;
        movingLayerRef.current.style.transition = 'transform 0.26s cubic-bezier(0.2, 0, 0, 1)';
      }
    });
  };
  ...
  const handleMouseLeave = () => {
    if (rafRef.current) cancelAnimationFrame(rafRef.current);
    if (!movingLayerRef.current) return;
    movingLayerRef.current.style.transform = `translate3d(0px, 0px, 0)`;
    movingLayerRef.current.style.transition = 'transform 0.7s cubic-bezier(0.25, 1, 0.5, 1)';
  };

  return (
    <div className="flex flex-col items-center justify-center min-h-0 -translate-y-[15px] md:translate-y-0">
      <span className="font-['Brygada_1918'] font-semibold tracking-widest uppercase text-[#571723] text-[16px] sm:text-[24px] md:text-[24px] mb-2 md:mb-12 whitespace-nowrap" style={{ height: 'auto' }}>{title}</span>
      <div 
        ref={containerRef}
...
const GalleryCarousel = ({ images, id, title, initialIndex = 0 }: GalleryCarouselProps) => {
  ...
  const fontSize = useMemo(() => {
    if (!dimensions.width || !title) return 32;
    // Scaled down by 20%: from 0.88 to 0.704 (width * 0.88 * 0.80)
    const targetW = dimensions.width * 0.704;
    try {
      const canvas = document.createElement('canvas');
      const ctx = canvas.getContext('2d');
      if (ctx) {
        ctx.font = '700 100px "Brygada 1918", serif';
        const m = ctx.measureText(title);
        if (m.width > 0) {
          return Math.round((targetW / m.width) * 100);
        }
      }
    } catch {
      // fallback
    }
    return Math.round(targetW / (title.length * 0.62));
  }, [dimensions.width, title, fontLoaded]);
  ...
  const handleMouseMove = (e: ReactMouseEvent<HTMLDivElement>) => {
    ...
    const x = e.clientX - rect.left - rect.width / 2;
    const y = e.clientY - rect.top - rect.height / 2;
    // Calculate a very subtle, soft pull (reduced range to prevent aggressive shifts)
    const pullX = Number((x * 0.02).toFixed(2));
    const pullY = Number((y * 0.02).toFixed(2));
    ...
    rafRef.current = requestAnimationFrame(() => {
      if (containerRef.current) {
        ...
        containerRef.current.style.transition = 'transform 0.85s cubic-bezier(0.16, 1, 0.3, 1)';
      }
    });
  };
  ...
  const handleMouseLeave = () => {
    ...
    containerRef.current.style.transition = 'transform 0.9s cubic-bezier(0.25, 1, 0.5, 1)';
  ...
  return (
    <>
      <div 
        ...
        className="relative w-full h-full rounded-xl overflow-hidden gallery-transition-container group select-none cursor-pointer border border-[#571723]"
      >
        ...
        {/* Placeholder overlay (50% of photo) */}
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-15">
          <div className="w-1/2 h-1/2 flex items-center justify-center">
            <img 
              src="/placeholder.png" 
              alt="Galeria placeholder" 
              className="w-full h-full object-contain select-none opacity-85" 
            />
          </div>
        </div>
        
        {/* Knockout Stencil Overlay (50% milky white with transparent cutout text, 88% card width) */}
        {title && dimensions.width > 0 && dimensions.height > 0 && (
          <svg 
            className="absolute inset-0 w-full h-full pointer-events-none z-[18] transition-opacity duration-500 ease-in-out group-hover:opacity-0"
            viewBox={`0 0 ${dimensions.width} ${dimensions.height}`}
            width="100%"
            height="100%"
          >
            <defs>
              <mask id={`knockout-mask-${id}`}>
                {/* Opaque white area shows the milky layer */}
                <rect x="0" y="0" width={dimensions.width} height={dimensions.height} fill="#ffffff" />
                {/* Black cutout text punches a completely transparent window through to the photo */}
                <text
                  ...
                  fontWeight="700"
                  fontSize={fontSize}
                  textLength={Math.round(dimensions.width * 0.704)}
                  lengthAdjust="spacing"
                >
                  {title}
                </text>
              </mask>
              {/* Soft blurred radial shadow behind the letters (reduced intensity and coverage by additional 20%) */}
              <radialGradient id={`text-shadow-grad-${id}`} cx="50%" cy="50%" r="50%">
                <stop offset="0%" stopColor="#000000" stopOpacity="0.30" />
                <stop offset="45%" stopColor="#000000" stopOpacity="0.21" />
                <stop offset="75%" stopColor="#000000" stopOpacity="0.10" />
                <stop offset="100%" stopColor="#000000" stopOpacity="0" />
              </radialGradient>
              {/* Gaussian blur filter for text-shaped dark aura (reduced intensity and coverage by additional 20%) */}
              <filter id={`blur-shadow-${id}`} x="-30%" y="-40%" width="160%" height="180%">
                <feGaussianBlur in="SourceAlpha" stdDeviation={Math.max(4.3, fontSize * 0.115)} result="BLURRED" />
                <feFlood floodColor="#000000" floodOpacity="0.30" result="COLOR" />
                <feComposite in="COLOR" in2="BLURRED" operator="in" />
              </filter>
            </defs>

            {/* Dark vignette aura behind and around the text area (coverage reduced by additional 20%) */}
            <ellipse 
              cx={dimensions.width / 2} 
              cy={dimensions.height / 2} 
              rx={dimensions.width * 0.27} 
              ry={Math.max(fontSize * 0.94, dimensions.height * 0.16)} 
              fill={`url(#text-shadow-grad-${id})`} 
            />

            {/* Blurred shadow matching text contour */}
            <text
              x={dimensions.width / 2}
              y={dimensions.height / 2}
              textAnchor="middle"
              dominantBaseline="central"
              filter={`url(#blur-shadow-${id})`}
              fontFamily="'Brygada 1918', serif"
              fontWeight="700"
              fontSize={fontSize}
              textLength={Math.round(dimensions.width * 0.704)}
            ...
            {/* 50% translucent milky white sheet with the text cut out */}
            <rect 
              x="0" 
              y="0" 
              ...
              fill="#ffffff" 
              fillOpacity="0.50" 
              mask={`url(#knockout-mask-${id})`} 
            />
          </svg>
        )}
        
        {/* Image Indicators */}
        <div className="absolute bottom-3 md:bottom-5 left-1/2 -translate-x-1/2 z-30 flex gap-2 md:gap-3 pointer-events-auto">
          {images.map((_, idx) => (
            <button 
              key={idx}
              onClick={(e) => {
                ...
                <div className="relative flex items-center justify-center">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-45">
                    <div className="w-1/2 h-1/2 flex items-center justify-center">
                      <img 
                        src="/placeholder.png" 
                        alt="Galeria powiększona placeholder" 
                        className="w-full h-full object-contain select-none opacity-85" 
                      />
                    </div>
                  </div>
...
function Home() {
  ...
  const [isContactMailOpen, setIsContactMailOpen] = useState(false);
  const [isContactMailSuccessOpen, setIsContactMailSuccessOpen] = useState(false);
  const [contactMailData, setContactMailData] = useState({
    subject: '',
    message: '',
    senderEmail: ''
  });
  ...
  return (
    <div className="bg-surface text-on-background font-body-md antialiased selection:bg-tertiary-container selection:text-on-tertiary-container overflow-x-hidden">
      ...
      <header className="hidden md:block fixed top-0 w-full z-50 backdrop-blur-md border-b border-outline-variant/20 bg-[#e9e0da] shadow-md transition-all duration-300" id="main-nav">
        <div className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop w-full">
          <div className="grid grid-cols-[1fr_auto_1fr] items-center w-full max-w-[1100px] mx-auto relative h-[80px]">
            <div className="flex items-center h-full relative">
              <a className="flex items-center justify-start w-[20px] overflow-visible group cursor-pointer translate-y-[3px] shrink-0" href="#" aria-label={t.back_to_top} onClick={(e) => { e.preventDefault(); window.scrollTo({ top: 0, behavior: 'smooth' }); }}>
              ...
              <div className="flex-1 flex items-center justify-center">
                <button 
                  ...
                  className="py-[3px] px-[7.5px] md:px-[9px] lg:px-3 rounded border border-[#571723] text-[#571723] font-label-sm text-[8.5px] md:text-[9px] lg:text-[10.5px] tracking-wider uppercase font-semibold hover:bg-[#571723] hover:text-[#e9e0da] transition-all duration-[1000ms] [transition-timing-function:cubic-bezier(0.4,0,0.2,1)] hover:scale-110 active:scale-110 will-change-transform inline-block cursor-pointer text-center whitespace-nowrap shrink-0 translate-y-[3px]"
                >
                  {t.book_table_upper}
                </button>
              </div>
            ...
            <nav className="flex gap-4 md:gap-6 lg:gap-8 justify-center items-center h-full">
              <a className="text-lg lg:text-xl font-semibold transition-all font-['Brygada_1918'] text-[#571723] nav-link-animated translate-y-[3px]" href="#about">{t.about_us}</a>
              <a className="text-lg lg:text-xl font-semibold transition-all font-['Brygada_1918'] text-[#571723] nav-link-animated translate-y-[3px]" href="#menu">{t.menu}</a>
              <a className="text-lg lg:text-xl font-semibold transition-all font-['Brygada_1918'] text-[#571723] nav-link-animated translate-y-[3px]" href="#gallery">{t.gallery}</a>
              <a className="text-lg lg:text-xl font-semibold transition-all font-['Brygada_1918'] text-[#571723] nav-link-animated translate-y-[3px]" href="#contact">{t.contact}</a>
            </nav>
            ...
            <div className="flex items-center justify-end h-full relative">
              ...
              <div className="flex gap-2 items-center justify-end text-[#571723] font-['Brygada_1918'] font-semibold text-xl translate-y-[3px] shrink-0">
      ...
      <section className="relative min-h-[calc(100dvh+50px)] md:min-h-screen flex items-center justify-center overflow-hidden bg-[#571723]" id="home">
        ...
        {/* Informacja placeholder na wideo (50% wielkości filmu) */}
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-[5]">
          <div className="w-1/2 h-1/2 flex items-center justify-center">
            <img 
              src="/placeholder.png" 
              alt="Wideo placeholder" 
              className="w-full h-full object-contain select-none opacity-85"
            />
          </div>
        </div>
        ...
        <div className="relative z-10 max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop flex flex-col items-center text-center w-full">
          <div className="w-full flex flex-col items-center justify-center animate-fade-in-up -translate-y-[30px] md:-translate-y-[15px]">
            <div className="relative w-full max-w-[341px] aspect-[341/324] mx-auto self-center overflow-hidden transition-transform duration-[1000ms] [transition-timing-function:cubic-bezier(0.4,0,0.2,1)] hover:scale-110 will-change-transform">
              <img 
                alt="NOLA Full Logo" 
                className="absolute w-[150.147%] max-w-none h-[111.728%] top-[-5.864%] left-[-24.633%] filter-beige select-none pointer-events-none" 
                src="https://lh3.googleusercontent.com/aida-public/AB6AXuDoFOQnNMMNHq6yoo_F1VB-RNYleHqut6evT1x7ZHw1Jz8P0w4xtOAOIZOWIMDaayIvZxXJqdV-6H_AvR9DhU1bpZTmjPSqBtUaIR55c0tB5N-m--I2woKXARO_Ft5lxB8q8XTrRa_4v4TInuWsJgmaHpbQgSUMD9ZzMc_RqGARmvdX-QiXZIRvUiX0FCiW4v6S5zk_43Gq7Np5npk3HjvYCl7wpAeNsSLFWRRr45At-MeCSy_uTp4Sz3zurUA8qjh_M33oKmzIeS0" 
              />
            </div>
            <button
              onClick={(e) => {
              ...
              className="mt-[100px] px-8 py-3 rounded-xl border border-[#e9e0da] text-[#e9e0da] font-label-sm uppercase hover:bg-[#e9e0da] hover:text-[#571723] transition-colors duration-300 flex items-center justify-center text-center mx-auto self-center cursor-pointer"
            >
              <span className="tracking-widest -mr-[0.1em]">{t.view_menu}</span>
            </button>
          </div>
        </div>
        <div className="absolute bottom-[65px] md:bottom-[25px] inset-x-0 w-full flex justify-center z-20 pointer-events-none translate-y-1/2">
          <button 
            className="btn-dalej flex flex-col items-center justify-center gap-[9.6px] mx-auto p-0 pointer-events-auto transition-transform duration-[1000ms] [transition-timing-function:cubic-bezier(0.4,0,0.2,1)] hover:scale-[1.15] active:scale-[1.15] will-change-transform bg-transparent border-none outline-none shadow-none cursor-pointer focus:outline-none appearance-none tap-highlight-transparent" 
          ...
          >
            <span className="font-label-sm text-xs uppercase text-white translate-y-1/2 text-center">{t.next}</span>
            <span className="material-symbols-outlined text-lg leading-none text-white text-center">keyboard_arrow_down</span>
          </button>
        </div>
      </section>
      ...
      <section className="relative overflow-hidden flex flex-col items-center justify-center h-[calc(100dvh+50px)] md:h-[calc(100vh-80px)] md:min-h-0 bg-[#571723] md:scroll-mt-[80px]" id="about">
        <div className="relative z-10 max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop w-full flex-1 flex flex-col pt-12 md:pt-24 pb-16 md:pb-20">
          ...
          <div className="flex-1 w-full flex flex-col justify-center items-center min-h-0">
            ...
            <div className="relative overflow-hidden flex items-center justify-center w-full min-h-0 max-h-[30vh] sm:max-h-[35vh] md:max-h-none rounded-xl -translate-y-[35px] md:translate-y-0">
              <div className="relative w-full h-full md:h-auto rounded-xl shadow-lg max-w-[270px] sm:max-w-[320px] md:max-w-none aspect-[4/3] overflow-hidden flex items-center justify-center">
                <img alt="Premium gourmet dish at NOLA restaurant" className="w-full h-full object-cover rounded-xl" src="https://lh3.googleusercontent.com/aida-public/AB6AXuAZQYiV4_z5KT5LpeN2Jhqs0o_e-L7WcHo-6iIH2zT4sVne6IbYossoc48-xH75lgcPkJTqXUUWqRXfT7XR8RhmRbHxcqNEKU8efLE3yWvFEUqQmIZbBOPSC9Ua037OU7GaRc91NanjK5qEv9M8LfULI_JooAcbSghnoxmCk6TP-oQrLwP30VwnA_ApjYusJmtNZMFz9QiGeEXuSNDVhKHWUJQnN25TaQoLzTbTljTiNEnFKYJmDSATiD0arwsvKjgH15ERJXlTAQo" />
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
                  <div className="w-1/2 h-1/2 flex items-center justify-center">
                    <img 
                      src="/placeholder.png" 
                      alt="O nas placeholder" 
                      className="w-full h-full object-contain select-none opacity-85" 
                    />
                  </div>
                </div>
        ...
        <div className="absolute bottom-[65px] md:bottom-8 left-0 w-full flex justify-center z-20 pointer-events-none translate-y-1/2">
          <button 
            className="btn-dalej flex flex-col items-center justify-center gap-[9.6px] mx-0 p-0 pointer-events-auto transition-transform duration-[1000ms] [transition-timing-function:cubic-bezier(0.4,0,0.2,1)] hover:scale-[1.15] active:scale-[1.15] will-change-transform bg-transparent border-none outline-none shadow-none cursor-pointer focus:outline-none appearance-none tap-highlight-transparent" 
            style={{ WebkitTapHighlightColor: 'transparent' }}
            onClick={(e) => {
              e.preventDefault();
              const el = document.getElementById('menu');
          ...
          >
            <span className="font-label-sm text-xs uppercase text-white translate-y-1/2">{t.next}</span>
            <span className="material-symbols-outlined text-lg leading-none text-white">keyboard_arrow_down</span>
          </button>
        </div>
      </section>

      {/* Menu Highlight Section / Interactive Gallery */}
      <section className="bg-[#e9e0da] relative overflow-hidden flex flex-col items-center justify-center h-[calc(100dvh+50px)] md:h-[calc(100vh-80px)] md:min-h-0 md:scroll-mt-[80px]" id="menu">
        <div className="relative z-10 max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop w-full flex-1 flex flex-col pt-12 md:pt-24 pb-16 md:pb-20 min-h-0">
          ...
          <div className="flex-1 w-full flex flex-col justify-center items-center min-h-0 mb-0 md:mb-0">
            <div className="flex-none flex flex-col md:flex-row justify-center items-center gap-6 sm:gap-8 md:gap-12 lg:gap-16 w-full section-interactive-area z-10 h-full min-h-0" id="interactive-plates">
              ...
              <InteractiveMenuPlate 
                title={t.food} 
                onClick={() => {
                  window.scrollTo(0,0);
                  navigate('/menu/food');
                }}
                maskImage="/plate-frame.svg"
              >
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[260px] sm:w-[350px] md:w-[465px] h-[260px] sm:h-[350px] md:h-[465px] flex items-center justify-center pointer-events-none z-10 transition-transform duration-500 group-hover:scale-105">
                  <img alt="Autorskie potrawy NOLA" className="hummus-plate w-full h-full rounded-xl object-cover absolute scale-[0.60] md:scale-[0.75] -translate-x-[0.45%] -translate-y-[1.45%]" id="hummus-plate-1" src="https://lh3.googleusercontent.com/aida-public/AB6AXuBGlh86wFS0wA6TGIChzSgFGV6OS7Ifs2E0VfHwOetD8VGmjqBkiDgoMLfKITZxPrq9H6P6ZHHUo1HWVccKWM5QGV8Rx6oPxu0GjI2W-NglPAq5WnyQi6Shi5Z4cPyk1ykL48pOTMl-GMVya7CU4GXtDhlmNWxwVPSJwui7I2Tj5F8iUW5jPsQDcKbp0OWrtO2nvVytDl7YOjGCHYC7JkIM-Oiht5SZX8LfA8UkUqJpmvA7N-V4L61jdvR5p-HnmCQryEN0Fon9fuc" />
                  <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-20 scale-[0.83] md:scale-[1.18] -translate-x-[0.45%] -translate-y-[1.45%]">
                    <div className="w-1/2 h-1/2 flex items-center justify-center">
                      <img 
                        src="/placeholder.png" 
                        alt="Potrawy placeholder" 
                        className="w-full h-full object-contain select-none opacity-85" 
                      />
                    </div>
                  </div>
              ...
              >
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[260px] sm:w-[350px] md:w-[465px] h-[260px] sm:h-[350px] md:h-[465px] flex items-center justify-center pointer-events-none z-10 transition-transform duration-500 group-hover:scale-105">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-20 scale-[0.83] md:scale-[1.18]">
                    <div className="w-1/2 h-1/2 flex items-center justify-center">
                      <img 
                        src="/placeholder.png" 
                        alt="Koktajle placeholder" 
                        className="w-full h-full object-contain select-none opacity-85" 
                      />
                    </div>
                  </div>
        ...
        <div className="absolute bottom-[65px] md:bottom-8 left-0 w-full flex justify-center z-20 pointer-events-none translate-y-1/2">
          ...
          >
            <span className="font-label-sm text-xs uppercase text-[#571723] translate-y-1/2">{t.next}</span>
            <span className="material-symbols-outlined text-lg leading-none text-[#571723]">keyboard_arrow_down</span>
          </button>
        </div>
      </section>

      {/* Decorative Divider */}
      <div className="w-full relative z-20 h-0 pointer-events-none">
        <div className="absolute top-0 left-0 w-full flex flex-nowrap justify-center -translate-y-1/2">
          {Array.from({ length: 150 }).map((_, i) => (
            <div key={`divider-circle-${i}`} className="w-[30px] h-[30px] rounded-full bg-[#571723] shrink-0" />
          ))}
        </div>
      </div>
      ...
      <section className="text-center relative overflow-hidden flex flex-col items-center justify-center h-[calc(100dvh+50px)] md:h-[calc(100vh-80px)] md:min-h-0 bg-[#571723] md:scroll-mt-[80px]" id="contact">
        ...
        <div className="relative z-10 max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop w-full flex-1 flex flex-col pt-12 md:pt-24 pb-16 md:pb-20 min-h-0">
          ...
          <div className="flex-1 w-full flex flex-col justify-center items-center min-h-0">
            <div className="grid grid-cols-1 lg:flex lg:flex-row lg:items-center lg:justify-center lg:gap-12 w-full max-w-[1140px] mx-auto h-full min-h-0">
              {/* Column 1: Logo & Address */}
              <div className="flex flex-col items-center md:items-end justify-start text-center md:h-[380px] py-0 md:py-1 w-full h-full min-h-0 shrink lg:w-[340px] lg:flex-none">
                <div className="w-[180px] md:w-[273px] flex flex-col items-center mx-auto md:mx-0">
                  <div className="flex items-center justify-center w-full relative">
                    <button 
                      onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} 
                      className="bg-transparent border-none p-0 outline-none m-0 shadow-none cursor-pointer focus:outline-none appearance-none tap-highlight-transparent relative block w-full aspect-[341/324] overflow-hidden hover:opacity-90 transition-opacity"
                      style={{ WebkitTapHighlightColor: 'transparent' }}
                      aria-label={t.back_to_top}
                    >
                      <img 
                  ...
                  <div className="text-[#e9e0da] font-body-md text-[11px] md:text-lg opacity-90 text-center w-full leading-tight md:leading-relaxed mt-2 md:mt-[42px] flex flex-col items-center justify-center shrink-0">
            ...
            <div className="flex flex-col items-center justify-center md:h-[380px] w-full h-full min-h-0 shrink -translate-y-[15px] md:translate-y-0 lg:w-[340px] lg:flex-none">
              <div className="flex flex-col items-center justify-center w-full">
                <div className="grid grid-cols-[auto_auto] gap-x-2.5 md:gap-x-4 mx-auto w-max text-[#e9e0da]">
                  {/* Headers */}
                  <div className="col-span-2 text-center font-display-lg text-[16px] md:text-2xl font-bold tracking-widest uppercase pb-1 md:pb-5">
                    {t.hours_open_1} {t.hours_open_2}
                  </div>
                  ...
                  <div className="text-left font-body-md text-[11px] md:text-lg opacity-90 whitespace-nowrap leading-tight md:leading-[18px] pb-0.5 md:pb-2.5">
                    <div className="inline-flex items-center tabular-nums">
                      <span>13:00</span>
                      <span className="w-3.5 md:w-5 inline-flex items-center justify-center text-center select-none shrink-0">–</span>
                      <span>23:00</span>
                    </div>
                  </div>
                  ...
                  {/* Row 4 */}
                  <div className="text-right font-body-md text-[11px] md:text-lg opacity-90 whitespace-nowrap leading-tight md:leading-[18px] pb-0">{t.sunday}</div>
                  <div className="text-left font-body-md text-[11px] md:text-lg opacity-90 whitespace-nowrap leading-tight md:leading-[18px] pb-0">
                    <div className="inline-flex items-center tabular-nums">
                      <span>12:00</span>
                      <span className="w-3.5 md:w-5 inline-flex items-center justify-center text-center select-none shrink-0">–</span>
                      <span>22:00</span>
                ...
                <div className="w-full flex justify-center mt-3 md:mt-[75px] text-center">
                  <button 
                    onClick={(e) => {
                      e.preventDefault();
                      setIsReservationOpen(true);
                    }}
                    className="py-2.5 sm:py-3 md:py-3 rounded-lg md:rounded-xl border border-[#e9e0da] text-[#e9e0da] font-label-sm text-[14px] sm:text-[15px] md:text-base tracking-widest uppercase hover:bg-[#e9e0da] hover:text-[#571723] transition-all duration-[1000ms] [transition-timing-function:cubic-bezier(0.4,0,0.2,1)] hover:scale-110 active:scale-110 will-change-transform inline-block cursor-pointer text-center whitespace-nowrap shrink-0 w-[220px] md:w-[280px]"
                  >
                    {t.book_table_upper}
                  </button>
                </div>
            ...
            {/* Column 3: Map */}
            <div className="flex flex-col items-center justify-center w-full h-full py-0 min-h-0 shrink -translate-y-[15px] md:translate-y-0 lg:w-[340px] lg:flex-none">
              <div className="relative w-[220px] h-[174px] md:w-full md:max-w-[340px] md:h-[380px] rounded-xl border border-[#e9e0da]/20 overflow-hidden shadow-lg mx-auto shrink-0 -translate-y-[10px] md:translate-y-0 lg:max-w-[340px] lg:h-[380px]">
          ...
          <div className="flex gap-8 sm:gap-24 w-full max-w-[600px] justify-center mx-auto mt-2 md:mt-12 pointer-events-auto items-center shrink-0 -translate-y-[15px] md:translate-y-0">
            <button 
              ...
              onClick={(e) => {
                e.preventDefault();
                setIsContactMailOpen(true);
              }}
              className="text-[#e9e0da] transition-all duration-300 transform hover:scale-110 flex items-center justify-center w-[26px] h-[26px] md:w-[32px] md:h-[32px] cursor-pointer bg-transparent border-none p-0" 
              aria-label="Email"
            >
              <Mail className="w-full h-full" strokeWidth={1.5} />
            </button>
            <a 
              className="text-[#e9e0da] transition-all duration-300 transform hover:scale-110 flex items-center justify-center w-[26px] h-[26px] md:w-[32px] md:h-[32px]" 
              href="https://www.google.com/search?q=NOLA+Restauracja+%26+Cocktail+Bar+Toru%C5%84" 
              target="_blank" 
            ...
            <a 
              className="text-[#e9e0da] transition-all duration-300 transform hover:scale-110 flex items-center justify-center w-[26px] h-[26px] md:w-[32px] md:h-[32px]" 
              href="https://www.facebook.com/NolaTorun/" 
              target="_blank" 
              rel="noopener noreferrer" 
              aria-label="Facebook"
            >
            ...
            <a 
              className="text-[#e9e0da] transition-all duration-300 transform hover:scale-110 flex items-center justify-center w-[26px] h-[26px] md:w-[32px] md:h-[32px]" 
              href="https://www.instagram.com/nola_torun/" 
              target="_blank" 
              rel="noopener noreferrer" 
          ...
          <div className="absolute bottom-2 inset-x-0 px-margin-mobile md:px-margin-desktop pointer-events-none z-50">
            <div className="w-full max-w-[1100px] mx-auto flex justify-end">
              <Link to="/edytor" className="pointer-events-auto text-[#e9e0da]/40 text-xs transition-colors hover:text-[#e9e0da]">{t.edit_menu}</Link>
            </div>
          </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="bg-[#e9e0da] text-[#571723] w-full max-w-md p-8 rounded-xl relative shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            <button 
              ...
              className="absolute top-4 right-4 text-[#571723] transition-transform duration-[1000ms] [transition-timing-function:cubic-bezier(0.4,0,0.2,1)] hover:scale-125 active:scale-125 will-change-transform flex items-center justify-center p-1 rounded-full cursor-pointer"
            >
            ...
            <form onSubmit={async (e) => {
              ...
              try {
                ...
                fetch('/api/reserve', {
                  ...
                  body: JSON.stringify({ ...reservationData, origin: window.location.origin, lang }),
                }).catch(() => {});
              ...
              <div>
                ...
                <p className="text-left italic text-[11px] text-[#571723] mt-1.5 leading-snug">
                  {t.more_people_contact}
                </p>
              </div>
      ...
      {/* Contact Mail Modal Overlay */}
      {isContactMailOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setIsContactMailOpen(false)}></div>
          <div className="bg-[#e9e0da] text-[#571723] w-full max-w-md p-6 sm:p-8 rounded-xl relative shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            <button 
              type="button"
              onClick={() => setIsContactMailOpen(false)}
              className="absolute top-4 right-4 text-[#571723] transition-transform duration-[1000ms] [transition-timing-function:cubic-bezier(0.4,0,0.2,1)] hover:scale-125 active:scale-125 will-change-transform flex items-center justify-center p-1 rounded-full cursor-pointer"
              aria-label={t.back}
            >
              <span className="material-symbols-outlined">close</span>
            ...
            <h3 className="font-display-lg text-2xl mb-6 text-center" style={{ fontFamily: '"Brygada 1918", serif' }}>
              {t.write_to_us}
            </h3>
            ...
            <form 
              onSubmit={async (e) => {
                ...
                try {
                  ...
                  // Dispatch to backend email endpoint
                  fetch('/api/contact', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                  ...
                  setTimeout(() => {
                    setIsContactMailOpen(false);
                    setIsContactMailSuccessOpen(true);
                    if (submitBtn) {
                      submitBtn.disabled = false;
                      submitBtn.innerHTML = t.send_upper;
                    }
                    setContactMailData({ subject: '', message: '', senderEmail: '' });
                  }, 1000);
                } catch (err) {
                  setIsContactMailOpen(false);
                  setIsContactMailSuccessOpen(true);
            ...
            >
              {/* Recipient email */}
              <div>
                <label className="block font-label-sm tracking-widest uppercase mb-1 text-xs opacity-70">
                  {t.recipient_email}
                </label>
                <input 
                  type="email" 
                  readOnly
                  value="wojnar21@gmail.com"
                  onKeyDown={(e) => {
                    // Allow copy (Ctrl+C / Cmd+C) and select all (Ctrl+A / Cmd+A)
                    if ((e.ctrlKey || e.metaKey) && ['c', 'a'].includes(e.key.toLowerCase())) return;
                    e.preventDefault();
                  }}
                  className="w-full bg-white/60 border border-[#571723]/20 p-2.5 text-sm rounded focus:outline-none text-[#571723]/50 cursor-text select-all selection:bg-[#571723]/20 selection:text-[#571723]"
                />
              </div>
              ...
              <div>
                ...
                <input 
                  type="email" 
                  required
                  value={contactMailData.senderEmail}
                  onChange={(e) => setContactMailData({ ...contactMailData, senderEmail: e.target.value })}
                  placeholder={t.email_placeholder}
                  className="w-full bg-white/60 border border-[#571723]/20 p-2.5 text-sm rounded focus:outline-none focus:border-[#571723] placeholder:text-[#571723]/50"
                />
              </div>
              ...
              <div>
                ...
                <input 
                  type="text" 
                  required
                  value={contactMailData.subject}
                  onChange={(e) => setContactMailData({ ...contactMailData, subject: e.target.value })}
              ...
              <div>
                ...
                <textarea 
                  required
                  rows={4}
                  value={contactMailData.message}
                  onChange={(e) => setContactMailData({ ...contactMailData, message: e.target.value })}
                  placeholder={t.email_content_placeholder}
                  className="w-full bg-white/60 border border-[#571723]/20 p-2.5 text-sm rounded focus:outline-none focus:border-[#571723] placeholder:text-[#571723]/50 resize-none"
                />
              </div>
              ...
              <div className="pt-2">
                <button 
                  type="submit" 
                  disabled={!contactMailData.subject.trim() || !contactMailData.message.trim() || !contactMailData.senderEmail.trim()}
                  className="w-full bg-[#571723] text-[#e9e0da] py-3 rounded-xl font-label-sm tracking-widest uppercase transition-all duration-[500ms] hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer shadow-md"
                >
                  {t.send_upper}
                </button>
              </div>
      ...
      {/* Contact Mail Success Modal Overlay */}
      {isContactMailSuccessOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setIsContactMailSuccessOpen(false)}></div>
          <div className="bg-[#e9e0da] text-[#571723] w-full max-w-md p-8 rounded-xl relative shadow-2xl animate-in fade-in zoom-in-95 duration-200 text-center">
            <h3 className="font-display-lg text-2xl mb-4" style={{ fontFamily: '"Brygada 1918", serif' }}>{t.message_sent_title}</h3>
            <p className="mb-6 font-body-md text-sm opacity-90 leading-relaxed">
              {t.message_sent_desc}
            </p>
            <button 
              onClick={() => setIsContactMailSuccessOpen(false)}
              className="px-8 py-3 bg-[#571723] text-[#e9e0da] rounded-xl font-label-sm tracking-widest uppercase transition-opacity inline-block cursor-pointer"
            >
              {t.ok}
            </button>
      ...
      {/* Events Information Modal Overlay */}
      {isEventsOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setIsEventsOpen(false)}></div>
          <div className="bg-[#e9e0da] text-[#571723] w-full max-w-md p-8 rounded-xl relative shadow-2xl animate-in fade-in zoom-in-95 duration-200 text-center border border-[#571723]/20">
            <button 
            ...
            <h3 className="font-display-lg text-2xl mb-5" style={{ fontFamily: '"Brygada 1918", serif' }}>{t.events_modal_title}</h3>
            
            <div className="mb-7 text-sm md:text-base font-body-md opacity-90 leading-relaxed text-balance">
              <p>
                {t.events_modal_desc_1}
              </p>
            </div>

            <button 
              onClick={() => setIsEventsOpen(false)}
              className="px-8 py-2.5 bg-[#571723] text-[#e9e0da] rounded-xl font-label-sm tracking-widest uppercase transition-opacity hover:opacity-90 inline-block cursor-pointer"
            >
              {t.ok}
            </button>
          </div>
...
const seasonalRecommendations: Record<Language, { food: MenuSection; cocktails: MenuSection }> = {
  pl: {
    food: {
      category: "POLECAMY",
      description: "Wkładka sezonowa, propozycje polecane w tym momencie.",
      items: [
        {
          name: "GRATIN ZIEMNIACZANY Z MUSEM Z BOCZNIAKÓW",
          badges: ["VEGE"],
          description: "Złocisty gratin ziemniaczany / kremowy mus z boczniaków z nutą trufli /\nprażone orzechy pekan / piklowana czerwona cebula (250 g)",
          price: "31 PLN"
        },
        {
          name: "HUMMUS Z PIECZONYMI WINOGRONAMI",
          badges: ["VEGAN"],
          description: "Kremowa pasta z ciecierzycy i tahini / pieczone winogrona /\nprażona ciecierzyca w ziołach / karmelizowane pestki dyni / chrupiąca bagietka (300 g)",
          price: "34 PLN"
        }
      ]
    },
    cocktails: {
      category: "POLECAMY",
      description: "Wkładka sezonowa, koktajle polecane w tym momencie.",
      items: [
        {
          name: "NOLA",
  ...
  en: {
    food: {
      ...
      description: "Seasonal specials, proposals recommended at this moment.",
      items: [
        {
          name: "POTATO GRATIN WITH OYSTER MUSHROOM MOUSSE",
          badges: ["VEGE"],
          description: "Golden potato gratin / creamy oyster mushroom mousse with truffle notes /\ntoasted pecans / pickled red onion (250 g)",
          price: "31 PLN"
        },
        {
          name: "HUMMUS WITH ROASTED GRAPES",
          badges: ["VEGAN"],
          description: "Creamy chickpea and tahini spread / roasted grapes /\nherb-roasted chickpeas / caramelized pumpkin seeds / crispy baguette (300 g)",
          price: "34 PLN"
        }
      ]
    },
    cocktails: {
      category: "RECOMMENDED",
      description: "Seasonal specials, cocktails recommended at this moment.",
      items: [
        {
          name: "NOLA",
          description: "Prosecco (90ml), Passoa (60ml), vanilla syrup, fresh lime juice ; 11.5% VOL.",
          price: "34 PLN"
        },
        {
          name: "RASPBERRY LIME SPRITZ",
          description: "Prosecco (175ml), fresh lime juice, raspberries, raspberry puree, cane sugar, mint ; 7.5% VOL.",
          price: "46 PLN"
        }
      ]
    }
...
function MenuPage() {
  ...
  const activeSections = useMemo(() => {
    const baseSections = (isCocktails ? data.cocktails : data.food) || [];
    const seasonal = seasonalRecommendations[lang][isCocktails ? 'cocktails' : 'food'];
    return [seasonal, ...baseSections];
  }, [isCocktails, data, lang]);
  ...
  return (
    ...
    >
      {/* Fixed placeholder overlay staying centered in place while content scrolls (40% width of menu) */}
      ...
      >
        <div id="sticky-nav-header" className="sticky top-[5px] bg-white z-20 w-full max-w-3xl mx-auto pt-4 md:pt-10 pb-4 border-b border-[#571723]/20 shadow-[0_15px_15px_-15px_rgba(0,0,0,0.1)] mb-8 px-0 rounded-t-xl">
          <div className="relative flex items-center justify-center w-full mb-6 mt-2">
            <button 
              ...
              className="absolute left-0 -translate-x-[2.5px] md:-translate-x-[3.5px] text-[#571723] flex items-center gap-[2px] z-10 transition-transform duration-[1000ms] [transition-timing-function:cubic-bezier(0.4,0,0.2,1)] hover:scale-110 active:scale-110 will-change-transform origin-left cursor-pointer"
            >
              <span className="material-symbols-outlined text-[16px] md:text-[20px] leading-none">arrow_back</span>
              <span className="font-label-sm uppercase tracking-widest text-[8px] md:text-[10px] leading-none">{t.back}</span>
            </button>
            ...
            <div className="absolute right-0 flex gap-1.5 items-center text-[#571723] font-['Brygada_1918'] font-semibold text-xs md:text-sm z-10">
              <button onClick={() => setLang('pl')} className={`transition-opacity ${lang === 'pl' ? 'opacity-100 font-bold underline' : 'opacity-50 hover:opacity-80'}`}>PL</button>
              <span className="opacity-40">|</span>
              <button onClick={() => setLang('en')} className={`transition-opacity ${lang === 'en' ? 'opacity-100 font-bold underline' : 'opacity-50 hover:opacity-80'}`}>EN</button>
            </div>
          </div>
