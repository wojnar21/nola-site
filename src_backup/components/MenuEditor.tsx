...
export function MenuEditor() {
  ...
  return (
    <div className="bg-surface text-on-background min-h-screen p-8 font-body-md overflow-y-auto">
      <div className="max-w-5xl mx-auto bg-white p-8 shadow-xl rounded-2xl border border-outline-variant/20">
        <div className="flex justify-between items-center mb-8 pb-4 border-b">
          <h1 className="text-3xl font-display-lg text-[#571723]">Edytor Menu</h1>
          <div className="flex gap-4 items-center">
            {message && <span className="text-sm rounded p-2 bg-[#571723]/10 text-[#571723] font-bold">{message}</span>}
            <button onClick={() => navigate('/')} className="px-4 py-2 text-sm border rounded">Wróć</button>
            <button onClick={handleSave} className="bg-[#571723] text-white px-6 py-2 rounded shadow text-sm uppercase tracking-widest font-bold">Zapisz Zmiany</button>
          </div>
        </div>

        <p className="mb-8 p-4 bg-yellow-50 text-yellow-800 rounded border border-yellow-200">
